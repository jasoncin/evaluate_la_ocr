...
def get_info_from_gt_dict(_dict, script=[], ignore_formal_key=[], filter_vertical=True, filter_horizontal=False):
    ...
    for i, region in enumerate(region_list):
        ...
        if region['shape_attributes']['name'] == 'rect' and region['shape_attributes']["height"] / (region['shape_attributes']["width"] + 0.01) >= 2 and len(region['region_attributes']['text']) >= 2:
            if filter_vertical:
                continue
        elif filter_horizontal:
...
def _update_excel_file(list_result, out_dir, filter_vertical=True, filter_horizontal=True):
    ...
    result_filename = os.path.join(out_dir, os.path.basename(out_dir))
    if filter_horizontal:
        result_filename += '_filter_horizontal'
    if filter_vertical:
        result_filename += '_filter_vertical'
...
def _evaluate_dir(model, gt_dir, pred_dir, out_dir, image_root="", list_group_script=["key", "value"],
    ...
    if pred_dir.endswith("results") or pred_dir.endswith("results/"):
        ...
        print("Converting to OCR output format...")
    ...
    _update_excel_file(list_final_result, out_dir, filter_vertical=filter_vertical, filter_horizontal=filter_horizontal)
...
if __name__ == '__main__':
    ...
    # test_textseg()
    list_weight_path = [
        # "/home/jason/Work/Cinnamon/prj_flax_sumitomo_life_prod2_ai/weight/ocr_remote/Ratio-0.3_20230330_144211UTC_bestAF_8200_Epoch72.pt",
        # "/home/jason/Work/Cinnamon/prj_flax_sumitomo_life_prod2_ai/weight/ocr_remote/Ratio-0.3-100-hw_20230406_055831UTC_bestAC3_9440_Epoch28Fit0.pt",
        # "/home/jason/Work/Cinnamon/prj_flax_sumitomo_life_prod2_ai/weight/ocr_remote/Ratio-0.5_20230330_062040UTC_bestAC3_9456_Epoch61Fit0.pt",
        # "/home/jason/Work/Cinnamon/prj_flax_sumitomo_life_prod2_ai/weight/ocr_remote/Ratio-0.5_20230330_062040UTC_bestAF_8291_Epoch61Fit0.pt",
    ...
    for weight_path in list_weight_path:
        ...
        for info in LIST_INPUT_DATA:
            if info['IS_RUN']:
                ...
                _evaluate_dir(model, info['GT_DIR'], info['PRED_DIR'], out_dir=info['OUT_DIR'], image_root=info['IMAGE_DIR'], debug=info["DEBUG"], filter_vertical=info["FILTER_VERTICAL"], filter_horizontal=info["FILTER_HORIZONTAL"])
